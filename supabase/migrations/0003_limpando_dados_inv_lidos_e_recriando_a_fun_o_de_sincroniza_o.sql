-- Remove as linhas existentes que não possuem um codigo_grupo, pois são inválidas para a sincronização.
DELETE FROM public.unidades WHERE codigo_grupo IS NULL;

-- Agora, com os dados limpos, podemos aplicar as regras de forma segura.
...
-- Recria a função com a lógica correta para inserir ou atualizar unidades.
...
BEGIN
  -- Limpa e converte o número de telefone para bigint
  BEGIN
  ...
  VALUES (
    ...
    NOW(), p_created_at_matriz, p_updated_at_matriz
  )
  ...
  -- Loga a operação para auditoria
  INSERT INTO public.sync_audit_log (entity_type, entity_id, operation, raw_data)
