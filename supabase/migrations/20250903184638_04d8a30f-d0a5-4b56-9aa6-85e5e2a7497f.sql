...
DROP TYPE IF EXISTS public.turma_status_old CASCADE;
...
-- Add RLS policy for enrollment window validation
CREATE POLICY "Users can enroll in turma within enrollment window"
...
WITH CHECK (
  ...
  AND can_enroll_in_turma(auth.uid(), turma_id)
);

-- Migrate existing turmas to have proper status
UPDATE public.turmas 
SET status = 'inscricoes_abertas'
WHERE status = 'agendada' 
  AND enrollment_open_at IS NOT NULL 
  AND enrollment_close_at IS NOT NULL
  AND now() >= enrollment_open_at 
  AND now() < enrollment_close_at;
