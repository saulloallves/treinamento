...
DROP FUNCTION IF EXISTS treinamento.advance_turmas_close(timestamp with time zone);
DROP FUNCTION IF EXISTS treinamento.advance_turmas_open(timestamp with time zone);
DROP FUNCTION IF EXISTS treinamento.approve_admin_user(uuid);
DROP FUNCTION IF EXISTS treinamento.approve_collaborator(uuid, boolean);
DROP FUNCTION IF EXISTS treinamento.authenticate_with_role(text, text, text);
DROP FUNCTION IF EXISTS treinamento.backfill_users_unit_code();
DROP FUNCTION IF EXISTS treinamento.can_enroll_in_turma(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.can_user_access_course(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.conclude_turma(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.create_class_audit_log();
DROP FUNCTION IF EXISTS treinamento.enroll_student_in_class(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.ensure_admin_bootstrap();
DROP FUNCTION IF EXISTS treinamento.fill_enrollment_unit_code();
DROP FUNCTION IF EXISTS treinamento.find_franchisee_by_unit_code(text);
DROP FUNCTION IF EXISTS treinamento.force_close_turma_enrollments(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.get_email_by_phone(text);
DROP FUNCTION IF EXISTS treinamento.get_franchisee_position(text);
DROP FUNCTION IF EXISTS treinamento.get_franchisee_unit_codes(uuid);
DROP FUNCTION IF EXISTS treinamento.get_pending_admin_approvals();
DROP FUNCTION IF EXISTS treinamento.get_professor_accessible_turmas(uuid);
DROP FUNCTION IF EXISTS treinamento.get_professor_enabled_fields(uuid, text);
DROP FUNCTION IF EXISTS treinamento.get_system_settings();
DROP FUNCTION IF EXISTS treinamento.has_professor_permission(uuid, text, text);
DROP FUNCTION IF EXISTS treinamento.has_professor_turma_access(uuid, uuid, text);
DROP FUNCTION IF EXISTS treinamento.is_admin(uuid);
DROP FUNCTION IF EXISTS treinamento.is_professor(uuid);
DROP FUNCTION IF EXISTS treinamento.link_enrollments_on_signup();
DROP FUNCTION IF EXISTS treinamento.link_enrollments_on_user_creation();
DROP FUNCTION IF EXISTS treinamento.manage_class_status(uuid, class_status);
DROP FUNCTION IF EXISTS treinamento.recalc_enrollment_progress(uuid);
DROP FUNCTION IF EXISTS treinamento.reject_admin_user(uuid);
DROP FUNCTION IF EXISTS treinamento.start_turma(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.sync_nomes_unidades();
DROP FUNCTION IF EXISTS treinamento.sync_password_on_change();
DROP FUNCTION IF EXISTS treinamento.sync_user_password();
DROP FUNCTION IF EXISTS treinamento.trg_update_progress_on_attendance();
DROP FUNCTION IF EXISTS treinamento.update_collaboration_approvals_updated_at();
DROP FUNCTION IF EXISTS treinamento.update_course_lessons_count();
DROP FUNCTION IF EXISTS treinamento.update_job_positions_updated_at();
DROP FUNCTION IF EXISTS treinamento.update_live_participants_updated_at();
DROP FUNCTION IF EXISTS treinamento.update_nomes_unidades();
DROP FUNCTION IF EXISTS treinamento.update_system_settings(jsonb);
DROP FUNCTION IF EXISTS treinamento.update_updated_at_column();
DROP FUNCTION IF EXISTS treinamento.upsert_unidade_from_matriz(uuid, bigint, text, text, text, text, text, text, text, text, text, text, text, text, text, jsonb);
DROP FUNCTION IF EXISTS treinamento.user_can_access_turma(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.user_can_access_turma_enrollments(uuid, uuid);
DROP FUNCTION IF EXISTS treinamento.validate_unit_codes();
