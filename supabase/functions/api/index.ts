...
async function handleAuth(request, path) {
  if (path[1] === 'login' && request.method === 'POST') {
    ...
    const { data: adminUser } = await supabaseTreinamento.from('admin_users').select('*').eq('user_id', data.user.id).single();
    if (!adminUser) {
      return new Response(JSON.stringify({
        error: 'Access denied'
      }), {
  ...
  if (path[1] === 'me' && request.method === 'GET') {
    ...
    const { data: adminUser } = await supabaseTreinamento.from('admin_users').select('*').eq('user_id', user.id).single();
    return new Response(JSON.stringify({
      user,
      admin: adminUser
    }), {
...
// Units endpoints
async function handleUnidades(request, path) {
  const codigo = path[1];
  if (request.method === 'GET' && codigo && path[2] === 'colaboradores') {
    // GET /unidades/{codigo}/colaboradores
    const { data: users, error } = await supabaseTreinamento.from('users').select('*').eq('unit_code', codigo).eq('active', true);
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
  ...
  if (request.method === 'GET' && codigo) {
    // GET /unidades/{codigo}
    const { data: unit } = await supabase.from('unidades').select('*').eq('group_code', codigo).single();
    if (!unit) {
      return new Response(JSON.stringify({
...
// Users endpoints
async function handleUsuarios(request, path) {
  const userId = path[1];
  if (request.method === 'GET' && userId && path[2] === 'historico') {
    ...
    const { data: userRecord } = await supabaseTreinamento.from('users').select('id, email').eq('id', userId).single();
    const emailKey = userRecord?.email || userId;
    const { data: enrollments } = await supabaseTreinamento.from('enrollments').select(`
        *,
        courses (name, theme),
        certificates (*)
      `).eq('student_email', emailKey);
    const { data: progress } = await supabaseTreinamento.from('student_progress').select('*').in('enrollment_id', enrollments?.map((e)=>e.id) || []);
    return new Response(JSON.stringify({
      enrollments,
      progress
    }), {
  ...
  if (request.method === 'GET' && userId) {
    // GET /usuarios/{id}
    const { data: user } = await supabaseTreinamento.from('users').select(`
        *,
  ...
  if (request.method === 'GET' && request.url.includes('search')) {
    ...
    const { data: user } = await supabaseTreinamento.from('users').select('*').eq('cpf', cpf).single();
    return new Response(JSON.stringify(user), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
  ...
  if (request.method === 'POST') {
    ...
    const { data, error } = await supabaseTreinamento.from('users').insert([
      userData
    ]).select().single();
    if (error) {
      return new Response(JSON.stringify({
  ...
  if (request.method === 'PUT' && userId) {
    ...
    await verifyAuth(request);
    const userData = await request.json();
    const { data, error } = await supabaseTreinamento.from('users').update(userData).eq('id', userId).select().single();
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
...
// Courses endpoints
async function handleCursos(request, path) {
  ...
  if (request.method === 'GET' && !courseId) {
    ...
    let query = supabaseTreinamento.from('courses').select('*');
    if (tipo_usuario) {
  ...
  if (request.method === 'GET' && courseId === 'minhas-inscricoes') {
    ...
    const user = await verifyAuth(request);
    const { data: enrollments, error } = await supabaseTreinamento.from('enrollments').select(`
        id,
        course_id,
        progress_percentage,
        status,
    ...
    let countsByEnrollment = new Map();
    if (enrollmentIds.length > 0) {
      const { data: attRows } = await supabaseTreinamento.from('attendance').select('enrollment_id').in('enrollment_id', enrollmentIds);
      for (const row of attRows || []){
        const k = row.enrollment_id;
    ...
    const result = enrollments?.map((e, index)=>{
      ...
      const calculatedProgress = totalLessons > 0 ? Math.max(0, Math.min(100, Math.floor(attended * 100 / totalLessons))) : e.progress_percentage || 0;
      return {
        ...e,
  ...
  if (request.method === 'GET' && courseId === 'por-email') {
    ...
    const { data: allEnrollments } = await supabaseTreinamento.from('enrollments').select('student_email');
    ...
    const { data: enrollments, error } = await supabaseTreinamento.from('enrollments').select(`
      ...
      `).eq('student_email', email).order('created_at', {
      ascending: false
    });
    console.log('Resultado da busca:', {
      encontrados: enrollments?.length || 0,
      erro: error?.message
    });
    if (error) {
      return new Response(JSON.stringify({
  ...
  if (request.method === 'GET' && courseId === 'por-unidade') {
    ...
    const { data: rawEnrollments, error: enrollmentsError } = await supabaseTreinamento.from('enrollments').select('*').eq('unit_code', unitCode).order('created_at', {
      ascending: false
    });
    ...
    const courseIds = [
      ...new Set(rawEnrollments.map((e)=>e.course_id))
    ];
    const { data: coursesData } = await supabaseTreinamento.from('courses').select('id, name, lessons_count, generates_certificate, theme').in('id', courseIds);
  ...
  if (request.method === 'GET' && courseId && path[2] === 'aulas') {
    ...
    const { data: lessons } = await supabaseTreinamento.from('lessons').select('id, title, description, zoom_start_time, zoom_join_url, status, course_id').eq('course_id', courseId).not('zoom_start_time', 'is', null).gt('zoom_start_time', nowIso).order('zoom_start_time', {
      ascending: true
    });
  ...
  if (request.method === 'GET' && courseId) {
    // GET /cursos/{id}
    const { data: course } = await supabaseTreinamento.from('courses').select(`
        *,
        lessons (*)
  ...
  if (request.method === 'POST') {
    ...
    const { data, error } = await supabaseTreinamento.from('courses').insert([
      courseData
    ]).select().single();
    if (error) {
      return new Response(JSON.stringify({
  ...
  if (request.method === 'PUT' && courseId) {
    ...
    await verifyAuth(request);
    const courseData = await request.json();
    const { data, error } = await supabaseTreinamento.from('courses').update(courseData).eq('id', courseId).select().single();
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
  ...
  if (request.method === 'DELETE' && courseId) {
    ...
    await verifyAuth(request);
    const { data, error } = await supabaseTreinamento.from('courses').update({
      status: 'Inativo'
...
// Lessons endpoints
async function handleAulas(request, path) {
  const lessonId = path[1];
  if (request.method === 'POST') {
    // POST /aulas
    await verifyAuth(request);
    const lessonData = await request.json();
    const { data, error } = await supabaseTreinamento.from('lessons').insert([
      lessonData
    ]).select().single();
    if (error) {
      return new Response(JSON.stringify({
  ...
  if (request.method === 'PUT' && lessonId) {
    ...
    await verifyAuth(request);
    const lessonData = await request.json();
    const { data, error } = await supabaseTreinamento.from('lessons').update(lessonData).eq('id', lessonId).select().single();
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
  ...
  if (request.method === 'DELETE' && lessonId) {
    // DELETE /aulas/{id}
    await verifyAuth(request);
    const { error } = await supabaseTreinamento.from('lessons').delete().eq('id', lessonId);
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
...
async function handleInscricoes(request, path) {
  if (request.method === 'POST') {
    // POST /inscricoes
    const { usuario_id, curso_id } = await request.json();
    // Check if already enrolled
    const { data: existing } = await supabaseTreinamento.from('enrollments').select('id').eq('student_email', usuario_id) // Assuming usuario_id maps to email
    .eq('course_id', curso_id).single();
    if (existing) {
      return new Response(JSON.stringify({
    ...
    // Get user data
    const { data: user } = await supabaseTreinamento.from('users').select('*').eq('id', usuario_id).single();
    if (!user) {
      return new Response(JSON.stringify({
        error: 'User not found'
      }), {
    ...
    const { data, error } = await supabaseTreinamento.from('enrollments').insert([
      {
        course_id: curso_id,
  ...
  if (request.method === 'GET' && request.url.includes('validar')) {
    ...
    const { data: enrollment } = await supabaseTreinamento.from('enrollments').select('id').eq('student_email', usuario_id).eq('course_id', curso_id).single();
    return new Response(JSON.stringify({
      enrolled: !!enrollment,
  ...
  if (request.method === 'GET' && path[1] === 'curso') {
    ...
    const { data: enrollments } = await supabaseTreinamento.from('enrollments').select('*').eq('course_id', courseId);
...
async function handlePresencas(request, path) {
  if (request.method === 'POST') {
    ...
    try {
      ...
      if (attendanceData.email && !attendanceData.user_id) {
        ...
        const { data: user, error: userError } = await supabaseTreinamento.from('users').select('id, email, name').eq('email', attendanceData.email).maybeSingle();
        console.log('=== DEBUG: Resultado da consulta users:', {
          user,
          userError
        });
        let foundUser = user;
        ...
        if (!foundUser) {
          ...
          const { data: enrollments, error: enrollmentError } = await supabaseTreinamento.from('enrollments').select('user_id, student_email, student_name').eq('student_email', attendanceData.email).limit(1);
          const enrollment = enrollments?.[0];
          console.log('=== DEBUG: Resultado da consulta enrollments:', {
            enrollment,
            enrollmentError
          });
          if (enrollment) {
            if (enrollment.user_id) {
              ...
              const { data: linkedUser, error: linkedError } = await supabaseTreinamento.from('users').select('id, email, name').eq('id', enrollment.user_id).maybeSingle();
              console.log('=== DEBUG: Resultado da busca do usuário linkado:', {
                linkedUser,
                linkedError
              });
              if (linkedUser) {
              ...
              const { data: newUser, error: createError } = await supabaseTreinamento.from('users').insert({
                name: enrollment.student_name || enrollment.student_email,
                email: enrollment.student_email,
                user_type: 'Aluno',
                active: true
              }).select('id, email, name').single();
              console.log('=== DEBUG: Resultado da criação do usuário:', {
                newUser,
                createError
              });
              if (newUser && !createError) {
                // Linkar o enrollment ao novo usuário
                await supabaseTreinamento.from('enrollments').update({
                  user_id: newUser.id
      ...
      if (!attendanceData.enrollment_id && attendanceData.lesson_id) {
        ...
        const { data: lesson } = await supabaseTreinamento.from('lessons').select('course_id').eq('id', attendanceData.lesson_id).single();
        if (lesson) {
          // Agora buscar o enrollment usando user_id + course_id
          const { data: enrollment } = await supabaseTreinamento.from('enrollments').select('id').eq('user_id', attendanceData.user_id).eq('course_id', lesson.course_id).maybeSingle();
          if (enrollment) {
      ...
      const { data, error } = await supabaseTreinamento.from('attendance').insert([
        attendanceData
      ]).select().single();
      if (error) {
        return new Response(JSON.stringify({
  ...
  if (request.method === 'GET' && path[1] === 'aula') {
    ...
    const { data: attendance } = await supabaseTreinamento.from('attendance').select(`
        *,
        users (name, email)
...
// Progress endpoints
async function handleProgresso(request, path) {
  if (request.method === 'GET') {
    ...
    const { data: enrollment } = await supabaseTreinamento.from('enrollments').select('progress_percentage, completed_lessons').eq('student_email', usuario_id).eq('course_id', curso_id).single();
  ...
  if (request.method === 'PUT') {
    ...
    const { data, error } = await supabaseTreinamento.from('enrollments').update({
      progress_percentage,
      completed_lessons: lesson_id ? [
        lesson_id
      ] : []
    }).eq('student_email', usuario_id).eq('course_id', curso_id).select();
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
...
// Quiz endpoints
async function handleQuiz(request, path) {
  const courseId = path[1];
  if (request.method === 'GET' && courseId) {
    // GET /quiz/{curso_id}
    const { data: questions } = await supabaseTreinamento.from('quiz').select('*').eq('course_id', courseId).order('order_index');
  ...
  if (request.method === 'POST' && courseId && path[2] === 'responder') {
    ...
    for (const resposta of respostas){
      const { data: question } = await supabaseTreinamento.from('quiz').select('correct_answer').eq('id', resposta.pergunta_id).single();
      const isCorrect = question?.correct_answer === resposta.resposta;
      if (isCorrect) correctCount++;
      // Save response
      const { data: response } = await supabaseTreinamento.from('quiz_responses').insert([
        {
          user_id: usuario_id,
          course_id: courseId,
...
async function handleCertificados(request, path) {
  if (request.method === 'POST' && path[1] === 'gerar') {
    ...
    const { data: enrollment } = await supabaseTreinamento.from('enrollments').select('progress_percentage').eq('id', enrollment_id).single();
    ...
    const { data, error } = await supabaseTreinamento.from('certificates').insert([
      {
        user_id: usuario_id,
        course_id: curso_id,
  ...
  if (request.method === 'GET' && path[1] && path[2]) {
    ...
    const { data: certificate } = await supabaseTreinamento.from('certificates').select('*').eq('user_id', usuario_id).eq('course_id', curso_id).single();
    if (!certificate) {
      return new Response(JSON.stringify({
  ...
  if (request.method === 'GET' && path[1] === 'emitidos') {
    ...
    await verifyAuth(request);
    const { data: certificates } = await supabaseTreinamento.from('certificates').select(`
        *,
        users (name, email),
        courses (name)
      `).order('generated_at', {
...
async function handleWhatsApp(request, path) {
  if (request.method === 'POST' && path[1] === 'disparo') {
    ...
    const { data, error } = await supabaseTreinamento.from('whatsapp_dispatches').insert([
      {
        type,
        item_id,
        item_name,
...
async function handleZoom(request, path) {
  // Helpers scoped to this handler to minimize file changes
  const getUserClient = (request)=>{
    const authHeader = request.headers.get('authorization') || '';
  ...
  if (request.method === 'POST' && path[1] === 'aulas' && path[2] === 'criar') {
    ...
    if (!curso_id || !titulo || !data || !hora || !duracao) {
      return new Response(JSON.stringify({
        error: 'Campos obrigatórios: curso_id, titulo, data, hora, duracao'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    try {
      ...
      if (!zRes.ok) {
        return new Response(JSON.stringify({
          error: 'Zoom create meeting failed',
          details: zJson
        }), {
          status: 502,
          headers: {
      ...
      const { data: orderRows } = await supabaseTreinamento.from('lessons').select('order_index').eq('course_id', curso_id).order('order_index', {
        ascending: false
      }).limit(1);
      const nextOrder = typeof body?.ordem === 'number' && body.ordem > 0 ? body.ordem : (orderRows && orderRows[0]?.order_index || 0) + 1;
      const { data: lesson, error } = await supabaseTreinamento.from('lessons').insert([
        {
          course_id: curso_id,
          title: titulo,
      ...
      return new Response(JSON.stringify({
        error: e.message || 'Erro ao criar aula no Zoom'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
  ...
  if (request.method === 'POST' && path[1] === 'aulas' && path[2] === 'sincronizar-presenca') {
    ...
    const { data: lesson, error: lerr } = await supabaseTreinamento.from('lessons').select('id, course_id, zoom_meeting_id').eq('id', lesson_id).single();
    if (lerr || !lesson?.zoom_meeting_id) {
      return new Response(JSON.stringify({
        error: 'Aula não encontrada ou sem zoom_meeting_id'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    try {
      ...
      if (!pRes.ok) {
        ...
        if (!pRes.ok) {
          return new Response(JSON.stringify({
            error: 'Falha ao obter participantes do Zoom',
            details: pJson
          }), {
            status: 502,
            headers: {
      ...
      for (const part of participants){
        ...
        const { data: enrollment } = await supabaseTreinamento.from('enrollments').select('id, student_email').eq('course_id', lesson.course_id).eq('student_email', email).single();
        if (!enrollment) continue;
        // Find user by email to get user_id
        const { data: user } = await supabaseTreinamento.from('users').select('id').eq('email', email).single();
        if (!user) continue;
        // Insert attendance
        const { error: aerr } = await supabaseTreinamento.from('attendance').insert([
          {
            user_id: user.id,
            lesson_id: lesson.id,
            enrollment_id: enrollment.id,
            attendance_type: 'automatica_zoom',
            confirmed_at: new Date().toISOString()
          }
        ]);
        if (!aerr) saved++;
        // Upsert student progress
        const { data: sp } = await supabaseTreinamento.from('student_progress').select('id').eq('enrollment_id', enrollment.id).eq('lesson_id', lesson.id).single();
        if (sp) {
          await supabaseTreinamento.from('student_progress').update({
            status: 'completed',
            completed_at: new Date().toISOString()
          }).eq('id', sp.id);
        } else {
          await supabaseTreinamento.from('student_progress').insert([
            {
              enrollment_id: enrollment.id,
              lesson_id: lesson.id,
              status: 'completed',
        ...
        const { count: totalLessons } = await supabaseTreinamento.from('lessons').select('*', {
          count: 'exact',
          head: true
        }).eq('course_id', lesson.course_id);
        const { count: completedCount } = await supabaseTreinamento.from('student_progress').select('*', {
          count: 'exact',
          head: true
        }).eq('enrollment_id', enrollment.id).eq('status', 'completed');
        ...
        const { data: enr } = await supabaseTreinamento.from('enrollments').select('completed_lessons').eq('id', enrollment.id).single();
        const setCompleted = Array.from(new Set([
          ...enr?.completed_lessons || [],
          lesson.id
        ]));
        await supabaseTreinamento.from('enrollments').update({
          progress_percentage: percentage,
          completed_lessons: setCompleted
        }).eq('id', enrollment.id);
      }
      return new Response(JSON.stringify({
        participantes: participants.length,
        presencas_registradas: saved
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
      ...
      return new Response(JSON.stringify({
        error: e.message || 'Erro ao sincronizar presenças'
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
...
function generateOpenApiSpec() {
  ...
  return {
    ...
    paths: {
      ...
      '/unidades/{codigo}': {
        get: {
          summary: 'Buscar unidade por código',
          parameters: [
            {
              name: 'codigo',
              in: 'path',
      ...
      '/unidades/{codigo}/colaboradores': {
        get: {
          summary: 'Listar colaboradores da unidade',
          parameters: [
            {
              name: 'codigo',
              in: 'path',
      ...
      '/presencas/aula/{aula_id}': {
        get: {
          summary: 'Presenças por aula'
        }
      },
      '/progresso': {
        get: {
          summary: 'Obter progresso'
        },
        put: {
      ...
      '/quiz/{curso_id}': {
        get: {
          summary: 'Perguntas do quiz'
        }
      },
      '/quiz/{curso_id}/responder': {
        post: {
      ...
      '/whatsapp/disparo': {
        post: {
          summary: 'Registrar e disparar mensagem (placeholder)'
        }
      },
      '/zoom/aulas/criar': {
        post: {
          summary: 'Criar reunião no Zoom e salvar aula'
        }
      },
      '/zoom/aulas/sincronizar-presenca': {
        post: {
          summary: 'Sincronizar presenças com Zoom'
        }
      }
    }
  };
}
async function handleDocs(request, path) {
