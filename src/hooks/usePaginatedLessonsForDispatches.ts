...
import { useLessonsForDispatches, LessonForDispatch } from './useLessonsForDispatches';

export interface PaginatedLessonsForDispatchesResult {
  lessons: LessonForDispatch[];
  currentPage: number;
  totalPages: number;
  totalItems: number;
  itemsPerPage: number;
  ...
  filteredLessons: LessonForDispatch[];
  courses: string[];
  isLoading: boolean;
}

export const usePaginatedLessonsForDispatches = () => {
  const { data: allLessons = [], isLoading } = useLessonsForDispatches();
